...
variable_longnames <- c(
  ...
  respiration_linear_bottom_up = 'Bottom-up estimate of time-varying linear component of respiration fluxes',
  respiration_linear_posterior = 'Posterior samples for time-varying linear component of respiration fluxes',
  ...
  samples_i <- perturbations %>%
    ...
    left_join(
      alpha %>%
        ...
        mutate(
          alpha_samples = alpha_samples[, floor(seq(1L, ncol(alpha_samples), length.out = n_samples))]
        ),
      by = 'basis_vector'
    ) %>%
...
ncatt_put(output_fn, 0, 'contact', 'Josh Jacobson <joshuah.jacobson@gmail.com>')
